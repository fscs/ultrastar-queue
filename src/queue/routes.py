...
async def add_song_to_queue(requested_song: UltrastarSong, singer: str, session: AsyncSession = Depends(get_session)):
    ...
    if not song_in_db:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Songdata not matching")
...
async def remove_song_from_queue(index: int):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Can not find index")
